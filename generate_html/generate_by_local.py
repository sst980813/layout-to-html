...
def _local_rewrite_html(
    ...
    *,
    ...
    user = textwrap.dedent(
        ...
        - 扁平数组为非树形：若原 block 有 children，则 block 本身不进入数组，仅将 children 的每个元素作为独立数组项；若 block 无 children，则该 block 本身进入数组。
        - 扁平数组中包含 x/y/width/height，可作为布局参考；你需要综合这些信息决定整体布局（单列/多列/网格/混合）以及局部元素排布方式。
