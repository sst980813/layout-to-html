...
def _blocks_to_flat_elements_json_for_prompt(blocks: List[Dict[str, Any]]) -> str:
    """
    将 blocks 转为元素数组 JSON（用于提示模型理解布局）。
    规则：
...
def _blocks_to_structure_xml_for_prompt(blocks: List[Dict[str, Any]]) -> str:
    ...
    for b in blocks:
        block_id = html_lib.escape(str(b.get("id", "")))
        block_type = html_lib.escape(str(b.get("type", "")))
        x = html_lib.escape(str(b.get("x", "")))
        y = html_lib.escape(str(b.get("y", "")))
        w = html_lib.escape(str(b.get("width", "")))
        h = html_lib.escape(str(b.get("height", "")))
        lines.append(
        ...
        if children:
            ...
            for ch in children:
                cid = html_lib.escape(str(ch.get("id", "")))
                kind = html_lib.escape(str(ch.get("kind", "")))
                cx = html_lib.escape(str(ch.get("x", "")))
                cy = html_lib.escape(str(ch.get("y", "")))
                cw = html_lib.escape(str(ch.get("width", "")))
                chh = html_lib.escape(str(ch.get("height", "")))
                if kind == "text":
                    txt = html_lib.escape(str(ch.get("content", "")))
                    sem = html_lib.escape(str(ch.get("semantic_level", "")))
                    fs = html_lib.escape(str(ch.get("font_size", "")))
                    lines.append(
                        f'        <node id="{cid}" kind="{kind}" x="{cx}" y="{cy}" width="{cw}" height="{chh}" semantic_level="{sem}" font_size="{fs}">{txt}</node>'
                    )
                elif kind == "image":
                    src = html_lib.escape(str(ch.get("image_path", "")))
                    lines.append(
            ...
            text_content = html_lib.escape(str(b.get("text_content", "")))
            img_path = html_lib.escape(str(b.get("image_path", "")))
            if text_content:
...
def _render_prompt_from_blocks(blocks: List[Dict[str, Any]]) -> str:
    ...
    user = textwrap.dedent(
        ...
        第一步：基于“结构 XML + 元素数组”进行布局规划（无需输出分析注释）
        - 你会同时收到结构 XML（用于理解层次关系）和元素数组 JSON（用于快速读取关键几何要素）。
        - 元素数组规则：若原 block 有 children，则 block 本身不进入数组，仅将 children 的每个元素作为独立数组项；若 block 无 children，则该 block 本身进入数组。
        - 元素数组中包含 x/y/width/height，可作为布局参考；你需要综合这些信息决定整体布局（单列/多列/网格/混合）以及局部元素排布方式。
